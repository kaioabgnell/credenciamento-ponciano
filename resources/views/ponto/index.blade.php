...
@section('content')
    ...
    <div class="page-header">
        <div>
            <h1 class="page-title">Histórico de Ponto</h1>
            <p class="page-subtitle">
                Registros de entrada e saída por data
                @if ($evento_id && $eventosLista->firstWhere('id', $evento_id))
                    <span
                        style="display:inline-flex;align-items:center;gap:5px;background:var(--roxo-light,#ede9fe);color:var(--roxo);border-radius:20px;padding:2px 10px;font-size:11px;font-weight:600;margin-left:8px">
    ...
    <div style="position: absolute;right: 28px;top: 93px;">
     <a href="{{ route('ponto.registrar') }}" class="btn btn-primary">⏱ Bater Ponto</a>
        <a href="{{ route(
            ...
            target="_blank" class="btn btn-secondary" title="Exportar listagem atual em PDF" style="white-space:nowrap">
            📄 Exportar PDF
        </a>
    </div>
    ...
    <form method="GET" class="filtros-bar">
        <input type="date" name="data" class="form-control" style="width:170px" value="{{ $data }}"
        ...
        <select name="empresa_id" class="form-control form-select" style="width:180px" onchange="this.form.submit()">
            ...
            @foreach ($empresas as $emp)
                <option value="{{ $emp->id }}" {{ $empresa_id == $emp->id ? 'selected' : '' }}>{{ $emp->nome }}
                </option>
            @endforeach
        </select>
                        ...
                        <tr id="row-ponto-{{ $ponto->id }}">
                            ...
                            <td>
                                @if ($ponto->funcionario->coordenador)
                                    <span
                                        style="background:#d1fae5; color:#065f46; border:1px solid #6ee7b7; padding:2px 10px; border-radius:20px; font-size:11.5px; font-weight:700; white-space:nowrap">⭐
                                        Sim</span>
                                @else
                                    <span
                                        style="background:#fff7ed; color:#c2410c; border:1px solid #fdba74; padding:2px 10px; border-radius:20px; font-size:11.5px; font-weight:600; white-space:nowrap">Não</span>
                                @endif
                            </td>
                            <td class="mono td-data" style="font-size:12.5px">{{ $ponto->data?->format('d/m/Y') ?? '—' }}
                            </td>
                            <td class="mono"
                                style="font-size:12.5px; font-weight:700; color:var(--azul-primario); letter-spacing:1px">
                                {{ $ponto->pulseira ?? '—' }}
                            </td>
                            ...
                            <td style="text-align:center; white-space:nowrap">
                                ...
                                <button class="btn-icon" title="Excluir" onclick="excluirPonto({{ $ponto->id }})"
    ...
    <div class="modal-overlay" id="modal-editar-ponto">
        <div class="modal-box" style="max-width:420px">
            ...
            <div style="font-size:16px; font-weight:700; color:var(--cinza-800); margin-bottom:20px">Editar Registro de
                Ponto</div>
            ...
            <div style="display:flex; flex-direction:column; gap:14px; text-align:left">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px">
                    <div class="form-group" style="margin:0">
                        ...
                        <input type="text" id="edit-entrada" class="form-control" placeholder="HH:MM" maxlength="5"
                            style="font-size:17px; font-weight:600; text-align:center; letter-spacing:2px">
                    </div>
                    <div class="form-group" style="margin:0">
                        <label class="form-label">🔴 Saída <span
                                style="font-weight:400; color:var(--cinza-400)">(opcional)</span></label>
                        <input type="text" id="edit-saida" class="form-control" placeholder="HH:MM" maxlength="5"
                            style="font-size:17px; font-weight:600; text-align:center; letter-spacing:2px">
                    </div>
                </div>
                <div id="erro-modal-editar"
                    style="display:none; font-size:12px; color:var(--vermelho); font-weight:500; padding:8px; background:var(--vermelho-light); border-radius:6px">
                </div>
            </div>
...
@push('scripts')
    <script>
        ...
        window.salvarEdicaoPonto = function() {
            ...
            if (!data) {
                mostrarErroModal('Informe a data.');
                return;
            }
            if (!/^\d{2}:\d{2}$/.test(entrada)) {
                mostrarErroModal('Entrada inválida. Use HH:MM.');
                return;
            }
            if (saida && !/^\d{2}:\d{2}$/.test(saida)) {
                mostrarErroModal('Saída inválida. Use HH:MM.');
                return;
            }
            ...
            confirmar({
                ...
                onConfirm: function() {
                    $.ajax({
                        ...
                        .done(function() {
                            $(`#row-ponto-${id}`).fadeOut(300, function() {
                                $(this).remove();
                            });
